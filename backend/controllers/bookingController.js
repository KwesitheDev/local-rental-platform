...
const cancelBooking = async (req, res) => {
    try {
        ...
        if (!booking.renter.equals(renter.user.id))return res.status(403).json({error: 'Unauthorized Action'})
...
const getUserBookings = async (req, res) => {
    try {
        const bookings = await Booking.find({renter: renter.user.id}).populate('listing')
        res.json(bookings)
    } catch (error) {
